...
Controls:
  ...
  General:
    ...
    - S: save map to JSON (Shift+S: Save As...), L: load map from JSON
"""
...
# Standard desktop save workflow: maps live in a project "maps" folder and use
# a single canonical extension. MAPS_DIR is the default location the native
# Save dialog opens to; MAP_FILE_EXTENSION is appended when the user omits it.
...
def prompt_save_map_path(initial_dir=None, initial_name="untitled"):
    ...
    if not path:
        return None
    # Ensure the extension even if the dialog/platform didn't apply it.
    if os.path.splitext(path)[1] == "":
        path += MAP_FILE_EXTENSION
    return path
...
TOOLTIPS = {
    ...
    ACTION_SAVE: ("Save Map", "Write the map to disk",
                  "Saves to the current file (asks for one the first time)"),
    ACTION_SAVE_AS: ("Save Map As...", "Save to a new file",
                     "Always opens the Save dialog to pick a file"),
    ACTION_LOAD: ("Load Map", "Reload the map from disk", f"Loads from {MAP_SAVE_PATH}"),
    ACTION_UNDO: ("Undo", "Undo the last edit", "Ctrl+Z"),
    ACTION_REDO: ("Redo", "Redo the undone edit", "Ctrl+Shift+Z / Ctrl+Y"),
}
...
# Tool/button id -> icon filename (without .png). Buttons listed here render as
# icon tiles; if the PNG is missing/unloadable, get_icon() returns None and the
# button falls back to its text label. The undo/redo entries point at simple
# placeholder art (generated by tools/make_placeholder_icons.py) -- drop final
# art at the same filename to replace it.
TOOL_ICONS = {
    TOOL_SELECT: "MouseSelection-button",
    TOOL_NODE: "node-button",
    TOOL_ROAD: "roadpath-button",
    ACTION_UNDO: "undo-button",
    ACTION_REDO: "redo-button",
}
...
class Toolbar:
    ...
    def __init__(self, font):
        ...
        self.action_buttons = [
            (ACTION_UNDO, "Undo", True),
            (ACTION_REDO, "Redo", True),
            (ACTION_SAVE, "Save Map", True),
            (ACTION_SAVE_AS, "Save Map As...", True),
...
class RoadRenderer:
    ...
    def draw_road(self, network, road, geometry, line_color, surface_color, alpha,
        ...
        if not road.is_preview:
            ...
            # Heavy dark outline along both carriageway/strip edges. Open
            # polylines, so trimmed mouths stay open (the junction surface
            # covers the seam). The road surface itself stays a single flat
            # color -- no inner "rim" line is drawn inboard of the edge.
...
class InputController:
    ...
    def __init__(self, network, camera):
        ...
        # Undo/redo. A drag mutates objects live for visual feedback but is
        # collapsed into ONE MoveCommand on release (see MoveTransaction).
        # move_txn is non-None only while a potential drag is in progress.
        ...
        # to overwrite without re-asking.
    ...
    def _handle_key(self, event):
        # Undo/redo: Ctrl/Cmd+Z, Ctrl/Cmd+Shift+Z or Ctrl/Cmd+Y to redo.
        if event.key in (pygame.K_z, pygame.K_y) and (
                event.mod & (pygame.KMOD_CTRL | pygame.KMOD_META)):
            if event.key == pygame.K_y or (event.mod & pygame.KMOD_SHIFT):
                self.redo()
            else:
                self.undo()
            return
    ...
    # ------------------------------------------------------------------
    # Move transactions (one drag -> one undo step) + undo/redo
    # ------------------------------------------------------------------
    def _set_node_pos(self, node, state):
        node.x, node.y = state
    ...
    def _begin_node_move(self, node):
        ...
        the start/end positions are recorded, never the intermediate frames."""
        txn = MoveTransaction()
        txn.add(lambda n=node: (n.x, n.y),
                lambda s, n=node: self._set_node_pos(n, s))
        self.move_txn = txn

    def _begin_curve_move(self, road):
        """Open a move transaction for a road's control point (its curve_offset)
        about to be dragged."""
        txn = MoveTransaction()
    ...
    def _commit_move(self):
        """Close the active move transaction on mouse release, pushing exactly
        ONE MoveCommand if anything actually moved. A click with no movement
        builds an empty command and is discarded (no history entry)."""
        if self.move_txn is None:
            return
        command = self.move_txn.build_command()
        self.move_txn = None
        if command is not None:
            self.undo_stack.push(command)

    def _cancel_move(self):
        """Abort the active drag (e.g. Escape): revert every grabbed object to
        its start position and record nothing."""
        if self.move_txn is None:
            return
        self.move_txn.cancel()
        self.move_txn = None
        self.dragging_node = None
        self.dragging_control_point = False
        self.mousedown_node = None
    ...
    def undo(self):
        self.status_message = "Undo" if self.undo_stack.undo() else "Nothing to undo"

    def redo(self):
        self.status_message = "Redo" if self.undo_stack.redo() else "Nothing to redo"
    ...
    def _select_tool_left_down(self, world_pos):
        ...
        if self.selected_road is not None:
            ...
            if dist <= hit_radius:
                self._begin_curve_move(self.selected_road)
        ...
        existing = self.network.node_at(*world_pos, radius=hit_radius)
        if existing is not None:
            # Don't decide drag-vs-click yet; motion/up resolves it. The move
            # transaction opens now (snapshotting the start position) but emits
            # no command unless the node actually moves before release.
            self._begin_node_move(existing)
    ...
    # Save / load
    # ------------------------------------------------------------------
    def _write_map_to(self, filepath):
        """Low-level write: serialize the active RoadNetwork to `filepath` and
        bind it as the current map. Shared by Save and Save As; performs no
        path selection of its own."""
    ...
    def save_map(self):
        """Save: write to the current file if the map has one; otherwise fall
        back to Save As so the user picks a destination the first time."""
        if self.current_map_path:
            self._write_map_to(self.current_map_path)
        else:
            self.save_map_as()

    def save_map_as(self):
        """Save As: always open the OS-native Save dialog. On confirm, the
        chosen file becomes the current map; on cancel, nothing changes."""
        if self.current_map_path:
            initial_dir = os.path.dirname(self.current_map_path)
            initial_name = os.path.splitext(
                os.path.basename(self.current_map_path))[0]
        else:
            initial_dir, initial_name = MAPS_DIR, "untitled"
        filepath = prompt_save_map_path(initial_dir, initial_name)
        if not filepath:
            self.status_message = "Save cancelled"
            return
        self._write_map_to(filepath)
    ...
    def load_from_file(self, filepath=MAP_SAVE_PATH):
        ...
        # Fresh document: drop undo history (old node/road objects are gone)
        # and any half-open drag.
        self.move_txn = None
        self.undo_stack.clear()

        # The loaded file becomes the current map, so a subsequent Save
        # overwrites it directly (standard desktop open/save behavior).
        self.current_map_path = filepath
    ...
    def load_test_city(self):
        ...
        destination test city (in memory only, unsaved until the user saves).
        Mutates the existing RoadNetwork in place so all references
        (placement, traffic) stay valid."""
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    ...
                    if settings_clicked == "clock":
                        ...
                        if clicked is not None:
                            ...
                            elif item_id == ACTION_UNDO:
                                controller.undo()
                            elif item_id == ACTION_REDO:
                                controller.redo()
                            elif item_id == ACTION_SAVE:
                                controller.save_map()
                            elif item_id == ACTION_SAVE_AS:
                                controller.save_map_as()
